...
class Scraper:
    ...
    def connect_sql(self):
        self.conn = pyodbc.connect("DRIVER={};SERVER={};DATABASE={};UID={};PWD={}".format(
            "ODBC Driver 17 for SQL Server", self.IP, self.database, self.user, self.Pass))
    ...
    def purchase_orders(self, Bearer, table):
        ...
        while True:
            ...
            try:
                r = requests.get(
                    'https://api.tradegecko.com/purchase_orders?limit=250&page='+str(i), headers=headers, timeout=120)
            except:
                wait_time.sleep(5)
                r = requests.get(
                    'https://api.tradegecko.com/purchase_orders?limit=250&page='+str(i), headers=headers, timeout=120)
            ...
            for x in r.json()['purchase_orders']:
                ...
                try:
                    ...
                    cached_quantity = ''
                try:
                    procurement_ids = ", ".join(
                        [str(each) for each in x['procurement_ids']])
                except:
                    procurement_ids = ''
                try:
                    purchase_order_line_item_ids = ", ".join(
                        [str(each) for each in x['purchase_order_line_item_ids']])
                except:
                ...
                if str(id_) not in self.IDS:
                    self.cursor.execute("""
                                    ...
                                    """.format(table), id_, created_at, updated_at, billing_address_id, company_id, currency_id, stock_location_id, supplier_address_id, default_price_list_id,
                                        ...
                                        notes, order_number, payment_due_at, procurement_status, reference_number, status, tax_treatment, received_at, total, tags, source_id, cached_quantity,
                                        procurement_ids, purchase_order_line_item_ids, default_price_type_id, tax_type)
                    #print(f'inserting {id_} in {table}', end='\r')
            self.cursor.commit()
            i += 1
    ...
    def get_existing_purchase_order_line_items(self, table):
        ...
        self.cursor.execute(
            'SELECT purchase_order_id FROM ###.dbo.{}'.format(table))
    ...
    def purchase_order_line_items(self, Bearer, table):
        ...
        while True:
            ...
            try:
                r = requests.get(
                    'https://api.tradegecko.com/purchase_order_line_items?limit=250&page='+str(i), headers=headers, timeout=120)
            except:
                wait_time.sleep(5)
                r = requests.get(
                    'https://api.tradegecko.com/purchase_order_line_items?limit=250&page='+str(i), headers=headers, timeout=120)
    ...
    def variants(self, Bearer, table):
        ...
        while True:
            ...
            try:
                r = requests.get(
                    'https://api.tradegecko.com/variants?limit=250&page='+str(i), headers=headers, timeout=120)
            except:
                wait_time.sleep(5)
                r = requests.get(
                    'https://api.tradegecko.com/variants?limit=250&page='+str(i), headers=headers, timeout=120)
    ...
    def locations(self, Bearer, table):
        ...
        while True:
            ...
            try:
                r = requests.get(
                    'https://api.tradegecko.com/locations?limit=250&page='+str(i), headers=headers, timeout=120)
            except:
                wait_time.sleep(5)
                r = requests.get(
                    'https://api.tradegecko.com/locations?limit=250&page='+str(i), headers=headers, timeout=120)
    ...
    def get_locations_data(self, location_id, Bearer):
        headers = {'Content-type': 'application/json',
                   'Authorization': 'Bearer '+Bearer}
        try:
            r = requests.get(
                f'https://api.tradegecko.com/locations?limit=1&ids[]={location_id}&page=1', headers=headers, timeout=120)
        except:
            pass
        try:
            address = r.json()['locations'][0]['address1']
        except:
            address = None
        return address

    def get_stock_existing_variants(self, table):
        self.cursor.execute('DELETE FROM ###.dbo.{}'.format(table))
        self.cursor.execute(
            'SELECT variant_id FROM ###.dbo.{}'.format(table))
        self.stock_variants_IDS = [each[0] for each in self.cursor.fetchall()]

    def stock_levels(self, Bearer, table):
        i = 1
        while True:
            headers = {'Content-type': 'application/json',
                       'Authorization': 'Bearer '+Bearer}
            while True:
                try:
                    r = requests.get(
                        f'https://api.tradegecko.com/stock_levels?limit=50&page={i}', headers=headers, timeout=120)
                except:
                    r = None
                if r.status_code == 200:
                    ...
                    wait_time.sleep(10)
            if r.json()['stock_levels'] == []:
                break
            for x in r.json()['stock_levels']:
                for each in x['locations']:
                    try:
                        variant_id = x['variant_id']
                    except:
                        variant_id = ''
                    if str(variant_id) not in self.stock_variants_IDS:
                        try:
                            location_address = self.get_locations_data(
                                each['location_id'], Bearer)
                        except:
                            location_address = None
                        try:
                            committed_stock = each['committed']
                        except:
                            committed_stock = ''
                        try:
                            stock_on_hand = each['stock_on_hand']
                        except:
                            stock_on_hand = ''
                        try:
                            incoming_stock = each['incoming']
                        except:
                            incoming_stock = ''
                        #print(variant_id)

                        self.cursor.execute("""
                                    INSERT INTO ###.dbo.{} (variant_id, committed_stock, stock_on_hand,
                                    incoming_stock, Location)
                                    VALUES
                                    (?,?,?,?,?)
                                    """.format(table), variant_id, committed_stock, stock_on_hand,
                                            incoming_stock, location_address)
                        #print(f'inserting {variant_id} in {table}', end='\r')
                self.cursor.commit()
            i += 1
...
if __name__ == '__main__':
    ...
    d.get_stock_existing_variants('the_Oodie_stock_levels')
    d.stock_levels(the_Oodie, 'the_Oodie_stock_levels')
    ...
    a.get_stock_existing_variants('pupnaps_stock_levels')
    a.stock_levels(pupnaps, 'pupnaps_stock_levels')
    ...
    b.get_stock_existing_variants('swished_stock_levels')
    b.stock_levels(swished, 'swished_stock_levels')
    ...
    c.get_stock_existing_variants('calming_blankets_stock_levels')
    c.stock_levels(calming_blankets, 'calming_blankets_stock_levels')
